...
def show_items(inventory, item_type):
    if item_type == "monster":
        ...
        for item in inventory["monster"]:
            print(f"{monster_arr['id']}  | {monster_arr['type']:<14} | {monster_arr['ATK Power']:<9} | {monster_arr['DEF Power']:<9} | {monster_arr['HP']:<4} | {item['stok']:<4} | {item['harga']:<5}")
    elif item_type == "potion":
        print("ID | Type                | Stok | Harga")
        for item in inventory["potion"]:
            print(f"{item['id']}  | {item['type']:<20} | {item['stok']:<5} | {item['harga']:<5}")
    else:
        print("Tipe item tidak valid.")

# Fungsi untuk membeli item
def buy_item(inventory, item_type, item_id, qty, oc):
